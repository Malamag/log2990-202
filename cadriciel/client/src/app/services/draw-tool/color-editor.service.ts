...
export class ColorEditorService extends RectangleService {
  ...
  applyColor(e:Element|null){
    //console.log(this.isRightClick)
    if(e == null) return;
    let c = e.lastElementChild as HTMLElement;
    if(this.isRightClick){
      c.style.stroke = c.getAttribute("stroke")!="none"?this.chosenColor.secColor:"";
    }else{
      c.style.fill = c.getAttribute("fill")!="none"?this.chosenColor.primColor:"";
    }
  }
  ...
  down(position: Point, insideWorkSpace:boolean, isRightClick:boolean) {
    ...
    this.applyColor(this.retrieveItemUnderMouse(position));
